...
const validateUrl = (value, { required = false, max = 500, github = false } = {}) => {
	...
	if (url.protocol !== "https:" || (github && url.hostname !== "github.com")) throw new Error("URL_INVALID");
	return url.href;
};
...
export function validateApplication(input, config = {}) {
	...
	if (!title || title.length > Number(validation.titleMax ?? 80)
		|| !description || description.length > Number(validation.descriptionMax ?? 240)) throw new Error("TEXT_INVALID");
...
export function buildIssue(application) {
	...
	return {
		...
		body: [
			"### 检查清单", checks, "", "### 友链信息", "```json", JSON.stringify(data, null, 4), "```", "",
			"### 友链地址", application.friendsPage, "", "### 友链仓库（可选）", application.friendsRepo || "未提供",
		].join("\n"),
	};
}
...
export function extractIssueUrl(body = "") {
	...
	if (!match) return "";
	try { return normalizeSiteUrl(JSON.parse(match[1]).url); } catch { return ""; }
}
